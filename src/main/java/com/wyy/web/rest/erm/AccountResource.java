...
import com.wyy.domain.erm.PersistentToken;
import com.wyy.domain.erm.User;
import com.wyy.repository.erm.PersistentTokenRepository;
...
public class AccountResource {
    ...
    public ResponseEntity<?> registerAccount(@Valid @RequestBody ManagedUserVM managedUserVM, HttpServletRequest request) {
        ...
        return userRepository.findOneByLogin(managedUserVM.getLogin().toLowerCase())
            .map(user -> new ResponseEntity<>("账号已存在", textPlainHeaders, HttpStatus.BAD_REQUEST))
            .orElseGet(() -> userRepository.findOneByEmail(managedUserVM.getEmail())
                .map(user -> new ResponseEntity<>("邮箱已经存在", textPlainHeaders, HttpStatus.BAD_REQUEST))
                .orElseGet(() -> {
